...
<div class="fieldcontain ${hasErrors(bean: parkingSpaceInstance, field: 'owner', 'error')} ">
    ...
    <g:select id="owner" name="owner.id" from="${sistemadevagasdeestacionamento.User.list()}" optionKey="id"
              value="${parkingSpaceInstance?.owner?.id}" class="many-to-one" noSelection="['null': '']"/>
...
<div class="fieldcontain ${hasErrors(bean: parkingSpaceInstance, field: 'sector', 'error')} required">
    ...
    <g:select name="sector" from="${parkingSpaceInstance.constraints.sector.inList}" required=""
              value="${parkingSpaceInstance?.sector}" valueMessagePrefix="parkingSpace.sector"/>
